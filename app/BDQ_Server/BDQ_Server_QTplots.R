QT_plots <- function(input, sim_QTtable) {
  ...
  plot <- ggplot(dfForPlotQT, aes(x = time / 168, y = median, 
    ...
    geom_line(size = 1.2) +
    theme_bw() +
    labs(x = "Time after start of treatment (weeks)", y = ("QTcF interval (ms)")) +
    ggtitle("QTcF interval (ms)") +
...
QTd_plots <- function(input, sim_QTtable) {
  
  sim_QTtable <- sim_QTtable %>%
    group_by(ID, regimen) %>%  
    mutate(IPREDd = IPRED - IPRED[time == 0])  # subtract baseline (time 0) per subject
  
  dfForPlotQT <- sim_QTtable %>%
    ungroup() %>%
    group_by(time, regimen) %>%
    summarize(
      lower = quantile(IPREDd, probs = 0.05),
      median = quantile(IPREDd, probs = 0.5),
      upper = quantile(IPREDd, probs = 0.95)
    )
  
  # Set dynamic ylim BDQ
  if (input$nsim == 1 || input$IIV == "OFF") { # individual
    maxQT <- max(sim_QTtable$IPREDd)
    ymaxlimitsQT <- maxQT 
    
    minQT <- min(sim_QTtable$IPREDd)
    yminlimitsQT <- minQT 
  ...
  plot <- ggplot(dfForPlotQT, aes(x = time / 168, y = median, 
    ...
    geom_line(size = 1.2) +
    theme_bw() +
    labs(x = "Time after start of treatment (weeks)", y = ("Change from baseline QTcF interval (ms)")) +
    ggtitle("Change from baseline QTcF interval (ms)") +
    coord_cartesian(ylim = c(yminlimitsQT, ymaxlimitsQT), xlim = c(0, input$simtime)) +
    scale_x_continuous(breaks = xbreaks) +
    scale_color_manual(values = c("#A084B5", "#D65D61", "#44BE5F")) +
    scale_fill_manual(values = c("#A084B5", "#D65D61", "#44BE5F")) +
